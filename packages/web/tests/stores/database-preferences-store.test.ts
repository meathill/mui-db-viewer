...
describe('database-preferences-store', () => {
  ...
  it('toggleColumnPin 应在 pinnedColumns 中添加或移除字段', () => {
    ...
    store.toggleColumnPin('db-1', 'users', 'id');
    let state = useDatabasePreferencesStore.getState();
    expect(state.databases['db-1'].tablePreferences['users'].pinnedColumns).toEqual(['id']);

    store.toggleColumnPin('db-1', 'users', 'name');
    state = useDatabasePreferencesStore.getState();
    expect(state.databases['db-1'].tablePreferences['users'].pinnedColumns).toEqual(['id', 'name']);

    store.toggleColumnPin('db-1', 'users', 'id');
    state = useDatabasePreferencesStore.getState();
    expect(state.databases['db-1'].tablePreferences['users'].pinnedColumns).toEqual(['name']);
  });
});

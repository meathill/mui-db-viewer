...
describe('QueryPage', () => {
  ...
  it('URL 从带 session 切回无 session 时应清空当前会话消息', async () => {
    vi.mocked(api.databases.list).mockResolvedValue(mockDbs);
    vi.mocked(api.querySessions.get).mockResolvedValue({
      session: {
        id: 's-1',
        databaseId: '1',
        title: '历史会话',
    ...
    setMockSearchParams('session=s-1');

    const view = render(<QueryPage />);

    await screen.findByText('查看上周订单');

    setMockSearchParams('');
    view.rerender(<QueryPage />);

    await waitFor(() => {
      expect(useQueryStore.getState().currentSessionId).toBeNull();
    });
    await screen.findByText('开始 AI 查询');
    expect(screen.queryByText('查看上周订单')).toBeNull();
  });
